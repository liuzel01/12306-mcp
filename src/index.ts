    ...
    return new McpServer(
        ...
        },
        {
            ...
            instructions:
                ...
                '**接口选择：**\n' +
                '*   用户询问“时刻表”“列车时刻”“班次时间”或某趟车的完整停站时间时，优先使用 `get-train-schedule`。\n' +
...
registerTool(
    'get-train-schedule',
    '【时刻表首选接口】当用户询问“时刻表”“列车时刻”“班次时间”或某趟车的完整停站时间时，必须优先使用本工具。查询指定车次在指定日期的完整计划时刻表，包括始发站、终到站、经停站、到达时间、出发时间和停留时长。',
    {
        trainCode: z.string().trim().min(1).describe('车次，例如 G1。'),
        departDate: z
            .string()
